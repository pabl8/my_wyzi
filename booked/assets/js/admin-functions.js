...
;(function($, window, document, undefined) {
	...
	$win.on('load', function() {
		...
		$('body').on('click','.addCustomTimeslot',function(e){
			...
			if (thisTimeslot.length<2)
				thisTimeslot = thisTimeslot.clone().appendTo(timeslotsContainter).removeClass().addClass('booked-customTimeslot').show();
			else
				thisTimeslot = $('.booked-customTimeslotTemplate:last-child').clone().appendTo(timeslotsContainter).removeClass().addClass('booked-customTimeslot').show();
			var rand = Math.floor((Math.random() * 100000000) + 1);
			init_custom_timeslot_block(thisTimeslot,rand);
		...
		$('#bookedTimeslotsWrap').on('click','a.booked-add-timeslot',function(e){
			...
			if (thisButton.hasClass('button-primary')){
				...
				if (activeTab == '#Bulk'){
					...
					filledRequired = true;
					if($('.tsBulk .wy-input-required').length){
						$('.tsBulk .wy-input-required').each(function(){
							if(filledRequired&&''==$(this).val()){
					...
					if (startTime && endTime && interval && count && filledRequired){
					...
					filledRequired = true;
					if($('.tsSingle .wy-input-required').length){
						$('.tsSingle .wy-input-required').each(function(){
							if(filledRequired&&''==$(this).val()){
								filledRequired = false;
								return;
							}
					...
					if (startTime && endTime && count && filledRequired){
