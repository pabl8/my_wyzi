...
if(!class_exists('booked_plugin')) {
	class booked_plugin {
		...
		public function add_to_calendar_check($posts){
		    ...
		    global $post;
		    $found = is_object( $post ) && 'user-account' == $post->post_name;

		    if(!$found){
		 
				foreach ($posts as $pst):
				
					$profile_shortcode = stripos($pst->post_content, '[booked-profile');
					$appts_shortcode = stripos($pst->post_content, '[booked-appointments');
		...
		**********************/
		...
		public function admin_scripts() {
			...
			$user_page = WyzHelpers::is_calendar_page();
		...
		public function frontend_scripts() {
			...
			if ( $user_page ):
				...
				wp_enqueue_script('booked-tooltipster', BOOKED_PLUGIN_URL . '/assets/js/tooltipster/js/jquery.tooltipster.min.js', array(), '3.3.0', true);
				wp_enqueue_script('booked-calendar-popup', BOOKED_PLUGIN_URL . '/assets/js/jquery.bookedCalendarPopup.js', array(), BOOKED_VERSION);
				wp_register_script('booked-admin', BOOKED_PLUGIN_URL . '/assets/js/admin-functions.js', array('jquery-ui-datepicker'), BOOKED_VERSION);
		...
		public function front_end_scripts() {
			...
			if ( ( ! $post || ! has_shortcode( $post->post_content, 'booked-profile' ) ) &&  WyzHelpers::is_calendar_page() && ( '' == $business_data || ! $business_data->can_bookings ) )
				return;
			...
			$booked_js_vars = array(
				...
				'i18n_appt_required_fields' => esc_html__('Please enter your name, your email address' . (
					function_exists( 'wyz_get_option' ) && 'on' == wyz_get_option( 'terms-and-cond-on-off' ) ? ', accept the terms and consitions ' : ' '
					) . 'and choose a password to book an appointment.','wyzi-business-finder'),
