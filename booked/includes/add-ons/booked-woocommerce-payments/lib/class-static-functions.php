...
class Booked_WC_Functions {
		...
		if (
			...
			$quer = array(
				...
				'meta_query' => array(
					...
					array(
					 ...
					 array(
				     'key' => 'is_automatically_created_for_calendar',
				     'compare' => 'NOT EXISTS', 
				     'value' => '' 
				    ),
				),
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				...
				if ( WyzHelpers::is_calendar_page() ) {
					if ( isset( $_GET['business-calendar'] ) ) {
	...
	public static function booked_custom_field_data( $custom_field_data = array() ) {
		...
		/*if ( empty($custom_fields) ) { // This is checking original Custom Fields but it may contain price in custom_field_data
		...
		$calendar_id = self::get_calendar_id_from_post_request();
			...
			// if Custom Fields does not have Product assigned to this appointment or at least selected 
			//from drop sown but there is a price, we create a booking product
		if ($has_product == false && self::wyz_findKeyof_custom_value($custom_field_data,'price') && $calendar_id) {

			$business_id_of_this_calander = get_term_meta( $calendar_id, 'business_id', true );
			$business_author_id = WyzHelpers::wyz_the_business_author_id($business_id_of_this_calander);
			$appointment_title = isset($_POST['title']) ? $_POST['title'] : '';	
			$timestamp = isset($_POST['timestamp']) ? $_POST['timestamp'] : '';
			$timeslot = isset($_POST['timeslot']) ? $_POST['timeslot'] : '';
			$price = isset($_POST['price']) ? $_POST['price'] : '0';

			$post = array(
		    'post_author' => $business_author_id,
		    'post_content' => '',
		    'post_status' => "publish",
		    'post_title' => apply_filters( 'wyz_appointment_creation_product', $appointment_title . ' ' .date('m-d-Y',$timestamp). ' ' . $timeslot, $appointment_title, $business_author_id ),
		    'post_parent' => '',
		    'post_type' => "product",
			);
			...
			if($post_id){
				...
				update_post_meta( $post_id, 'business_id', $business_id_of_this_calander );
				update_post_meta( $post_id, 'is_automatically_created_for_calendar', 'yes' );

				// Assign Product to Vendor so he sees it in his Products List in WCMP Dashboard
				$vendor_term = get_user_meta($business_author_id, '_vendor_term_id', true);
				...
				$product = Booked_WC_Product::get($post_id);
				$field_marker = 'product_id::' . $post_id;
				$value_product_title = esc_html($product->title);

				$new_custom_array_added_to_custom_field_array = array(
					'label' => 'price' ,
					'value' =>  $value_product_title . "<!-- {$field_marker} -->",
					 );

				$custom_field_data[] = $new_custom_array_added_to_custom_field_array;
				$has_product = true;
			}
	...
	// function to search an array and get if key exists

	public static function wyz_findKeyof_custom_value($array, $keySearch)
	{
	    foreach ($array as $key => $item) {
	        if ($item['label'] == $keySearch) {
	            return true;
	        }
	    }
	    return false;
