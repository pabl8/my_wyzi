...
class Booked_WC_Product {
	...
	protected function _get_title() {
		...
		if ( $this->type === 'variable' ) {
			$this->title = $this->data->get_title();
		} else {
			$this_price = ( $this->data->get_price() ? $this->data->get_price() : '0' );
			switch ( $booked_wc_currency_position ) {
				case 'left' :
					$this->title = $booked_wc_currency_symbol . $this_price . ' - ' . $this->data->get_title();
				break;
				case 'right' :
				 	$this->title = $this_price . $booked_wc_currency_symbol . ' - ' . $this->data->get_title();
				break;
				case 'left_space' :
				  	$this->title = $booked_wc_currency_symbol . ' ' . $this_price . ' - ' . $this->data->get_title();
				break;
