		...
		if ( !empty($available_timeslots_array) ):
			foreach( $available_timeslots_array as $_timeslot => $_available ):
				...
				if ( $timeslot == $_timeslot ):
					$_available = '';
				elseif($_available>1000):
					$_available = ' (' . esc_html__( 'Unlimited Space Available', 'wyzi-business-finder' ) . ')';
				elseif ( $_available > 0 ):
...
function booked_render_single_timeslot_form($timeslot_intervals,$type = false){
	...
	if ( function_exists( 'booked_render_single_timeslot_form_override' ) )
		return booked_render_single_timeslot_form_override( $timeslot_intervals,$type );
	...
	echo '<form id="single-timeslot-form" action="" method="post">';
		...
		if (WyzHelpers::is_user_vendor(get_current_user_id()))
			echo booked_render_number_field('price', esc_html__('Price (optional)','wyzi-business-finder'));
...
function booked_render_bulk_timeslot_form($timeslot_intervals,$type = false){
	...
	if ( function_exists( 'booked_render_bulk_timeslot_form_override' ) )
		return booked_render_bulk_timeslot_form_override( $timeslot_intervals,$type );
...
function booked_render_number_field($field_name,$placeholder){
	$html = '<input type="number" name="'.$field_name.'" placeholder="'.$placeholder.'" />';
	return apply_filters( 'booked_text_field', $html );
}
			...
			<td>
				<table>
					<thead>
						<tr>
							<?php 
								$front_end_classes = !is_admin() ? "wyz-primary-color wyz-prim-color" : "";
								echo '<th data-day="'.$day_loop_english_array[$key].'">'.$day.'<a href="#" class="'.$front_end_classes.' booked-add-timeslot button">'.esc_html__('Add','wyzi-business-finder').'</a></th>'; ?>
						</tr>
					</thead>
					<tbody>
						<tr>
...
function booked_render_timeslot_info($time_format,$day,$time,$count,$calendar_id,$booked_defaults=array()){
	...
	$price = isset($booked_defaults[$day.'-details'][$time]['price']) ? $booked_defaults[$day.'-details'][$time]['price'] : '';
	ob_start();
	...
	echo '<span class="timeslot" data-timeslot="'.$time.'">';
		...
		echo '<span class="slotsBlock">';
			...
			if($count>1000)
				echo '<span class="count">' . esc_html__('Unlimited Space Available','wyzi-business-finder') . '</span>';
			else
		...
		if ( $price ) {
			echo '<span class="booked_slot_title">'.sprintf( esc_html__( 'Price: ','wyzi-business-finder') . get_woocommerce_price_format() , get_woocommerce_currency_symbol() , $price ).'</span>';
		}
...
if ( ! function_exists('booked_render_count_select')){
				...
				foreach($custom_fields as $field):
					...
					switch($field_type):
						...
						case 'paragraph-text-label' :
							...
							?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
								<small><?php esc_html_e('Paragraph here it comes Text','wyzi-business-finder'); ?></small>
								<p><input class="cf-required-checkbox"<?php if ($is_required): echo ' checked="checked"'; endif; ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php esc_html_e('Required Field','wyzi-business-finder'); ?></label></p>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this field...','wyzi-business-finder'); ?>" />
								<span class="cf-delete"><i class="booked-icon booked-icon-close"></i></span>
							</li><?php
