...
$price = isset($_POST['price']) ? $_POST['price'] : '0';	// Grabbed Price from Hidden Field in Appointment Form
...
if ($appt_is_available):
	...
	// Lets check if Price is Numberic and Not Zero, then we add price to Custom Fields so WooCommerce action booked_custom_field_data gets fired to pass price
	if (is_numeric($price) && $price != 0 ){

		$price_custom_name = array(
			'name'  => 'price',
			'value' => 'price',
			);
		$price_custom_value = array(
			'name'  => 'price',
			'value' => $price,
			);
		$custom_fields[] = $price_custom_name;
		$custom_fields[] = $price_custom_value;
	}
	...
	if (!empty($custom_fields)):
		...
		if (!empty($custom_field_data)):
			foreach($custom_field_data as $key => $data):
				$cf_meta_value .= '<p class="cf-meta-value"><strong>'.$data['label'].'</strong><br>'.wpautop($data['value']).'</p>';
			endforeach;
		endif;
	...
	if ($customer_type == 'guest'):
		...
		if ( $name_requirements == 'require_surname' && !$surname ):
			...
			if ($email && is_email($email) && $name || !$email && !$email_required && $name):
				...
				do_action( 'booked_after_post_meta_update', $post_id );
