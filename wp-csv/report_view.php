<h2><?php _e( 'Import Results', 'wyzi-business-finder' ); ?></h2>

<?php

if ( is_array( $info_messages ) && !empty( $info_messages ) ) {
	$message_heading = __( 'Details', 'wyzi-business-finder' );
...
if ( is_array( $error_messages ) && !empty( $error_messages ) ) {
	$message_heading = __( 'Errors', 'wyzi-business-finder' );
...
if ( is_array( $warning_messages ) && !empty( $warning_messages ) ) {
	$message_heading = __( 'Warnings', 'wyzi-business-finder' );
...
if ( empty( $error_html ) && empty( $warning_html ) ) {
	$success_message = __( 'Imported successfully, with no errors or warnings.', 'wyzi-business-finder' );
